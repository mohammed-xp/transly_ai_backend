...
- **التاسك المفتوحة:** 🚧 **TASK-011 — الكاش ميقدّمش ترجمة من موديل تاني** (اتكتبت 2026-08-27 **بطلب صريح منه**، وهو اللي اختار الاتجاه من تلات بدائل اتعرضت عليه). الملف: `mentor/tasks/TASK-011.md`.
- **اللي قبلها:** ✅ TASK-010 اتقفلت **واتكوميتت** — `4e893d7 feat: cache Gemini translations in MySQL to cut API calls (TASK-010)`. الـ working tree نضيف.
...
|---|---|---|---|---|
| TASK-011 | الكاش ميقدّمش ترجمة من موديل تاني — تثبيت الموديل + مصدر واحد لحقل `Model` + إبطال الكاش | M3 | 🚧 **مفتوحة** | — |
...
> **الحالة بتاريخ 2026-08-27 (آخر الجلسة):** TASK-008 ✅ · TASK-009 ❌ اتلغت · **TASK-010 ✅ اتقفلت واتكوميتت (`4e893d7`)** · **TASK-011 🚧 مفتوحة — مستنيين التسليم**.
>
> ## 🚧 TASK-011 مفتوحة — الجلسة الجاية = review
>
> **اتكتبت بطلب صريح منه ("ايش التاسك الجديدة؟؟")، وبعد ما اتعرضت عليه تلات اتجاهات واختار.** الاتجاه اللي اختاره: **تثبيت اسم الموديل + إبطال الكاش** (البديلين التانيين كانوا M4 error contract، والـ history endpoint).
>
> **الفخاخ المزروعة — تتراجع في الـ review:**
> - 🪤 **الـ unique index على `CacheKey` لوحده.** لو صلّح مسار الـ read (فلتر على الموديل) وساب الـ index زي ما هو → أول كتابة بموديل جديد بتوقع في `DbUpdateException`، والـ `catch` اللي **هو نفسه كتبه** في TASK-010 بيبلعها كـ `LogWarning` → **الكاش بيموت في صمت وكل request بيروح لـ Gemini**. ده بالظبط نمطه المسجّل: إجراء صح على نص المشكلة.
> - 🪤 **المكان التاني لـ `response.Model`** — `GeminiApiService.cs:140` بيرمي `UpstreamError` لو `response.Model` فاضي. لو نقل مصدر الـ `Success` للـ config وساب الـ guard، ترجمة **ناجحة** ممكن ترجع 502 بسبب حقل مبقاش ليه استعمال. نمط مسجّل: "بيصلّح الملاحظة في مكان وبيسيب المكان التاني المطابق".
> - 🪤 **مصير الصفوف القديمة** — مفيش إجابة واحدة صح، بس **الصمت = التاسك ناقصة**.
> - ❓ **الجزء التالت اللي بيحدد الناتج ومش في المفتاح** = **الـ prompt نفسه** (`GeminiApiService.cs:41-47`) — أول ما يعدّل صياغته، الكاش هيقدّم ترجمات اتولدت بـ prompt قديم. مش مطلوب يصلحه، مطلوب يشوفه.
>
> **الفكرة اللي التاسك بتحوم حواليها:** مفتاح الكاش لازم يشمل كل اللي بيحدّد الناتج، مش المدخلات بس.
>
