...
> اتغيّرت في commits من غير ticket (`4ccca52` · `ff7fdd2` · `07b472f`، 2026-09-22/23) ولسه متراجعتش. مش معروف لسه إذا كان تغيير مقصود للقرار ولا لأ.

| # | القرار | الكود النهاردة |
|---|---|---|
| 4 | `DateTimeOffset` مش `DateTime` | `ApiResponse.Timestamp` نوعه `DateTime` |
| 7 | `/v1/...` والـ `/api` تتضاف عند الـ proxy | الـ controllers على `api/v1/[controller]` |
| 11 | مفيش envelope | `ApiResponse<TData>` فيه `Success`/`StatusCode`/`Message`/`Data`/`Errors`/`Timestamp` |
| 15 | مفيش interface بـ implementation واحدة | `IAuthService` · `IGeminiApiService` · `ITranslationService` |
